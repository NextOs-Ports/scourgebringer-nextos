# ScourgeBringer — framework 6.8 port (AArch64 MonoGame so-loader)
...
It is the third Mono-Android game in this repository, after
`ports/stardewvalley` and `ports/tmntsr`; their proven loader lineage remains in
the game-specific adapter while the reusable host contracts now come from the
framework.
...
```bash
cd ports/scourgebringer
bash build.sh
```

`build.sh` extracts the immutable framework commit from `FRAMEWORK-PIN.json`,
checks every pinned component version, then runs in the pinned offline Debian
Buster container. It uses the NextOS sysroot for headers only and produces
...
É o terceiro jogo Mono-Android do repositório, depois de `ports/stardewvalley` e
`ports/tmntsr`; a linhagem comprovada do loader continua no adapter específico,
enquanto os contratos reutilizáveis do host agora vêm do framework.
...
O `build.sh` extrai o commit imutável declarado no `FRAMEWORK-PIN.json`, confere
a versão de cada componente fixado e só então roda no container Debian Buster
